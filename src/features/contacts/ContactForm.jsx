...
import LocalidadSearch from '@/components/ui/LocalidadSearch'
...
function ubicacionFromDefaults(defaultValues) {
  if (!defaultValues?.provincia) return null
  return {
    localidad: defaultValues.localidad,
    provincia: defaultValues.provincia,
    coords: defaultValues.mapa_coords,
  }
}

export function ContactForm({ defaultValues, onSubmit, loading, clientOptions = [] }) {
  const today = new Date().toISOString().split('T')[0]
  const [ubicacion, setUbicacion] = useState(ubicacionFromDefaults(defaultValues))
  ...
  useEffect(() => {
    if (defaultValues) {
      reset(defaultValues)
      setUbicacion(ubicacionFromDefaults(defaultValues))
    }
  }, [defaultValues, reset])
  ...
  function handleFormSubmit(data) {
    onSubmit({
      ...
      mapa_coords: ubicacion?.coords || null,
    })
  }
